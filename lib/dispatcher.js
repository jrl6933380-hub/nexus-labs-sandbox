...
export class MemoryDispatchStore {
  ...
  async complete({ taskId, agentId, leaseToken, result, now }) {
    const record = this.records.get(taskId);
    if (!record || record.owner !== agentId || record.lease_token !== leaseToken ||
        !ACTIVE_STATES.has(record.state)) return null;
...
export class RedisDispatchStore {
  ...
  async mutateOwned({ taskId, agentId, leaseToken, now, mutate, args = [] }) {
    ...
    const raw = await this.command(['EVAL', script, '1', key, agentId, leaseToken, String(now), ...args.map(String)]);
    return raw ? JSON.parse(raw) : null;
  }
  heartbeat({ taskId, agentId, leaseToken, now, leaseMs }) {
    return this.mutateOwned({ taskId, agentId, leaseToken, now,
      args: [now + leaseMs],
      mutate: "if tonumber(r.lease_expires_at or 0)<=tonumber(ARGV[3]) then return nil end r.state='running'; r.lease_expires_at=tonumber(ARGV[4]); r.updated_at=tonumber(ARGV[3]);" });
  }
  complete({ taskId, agentId, leaseToken, result, now }) {
    return this.mutateOwned({ taskId, agentId, leaseToken, now,
      args: [JSON.stringify(result ?? null)],
      mutate: "r.state='completed'; r.result=cjson.decode(ARGV[4]); r.owner=cjson.null; r.lease_token=cjson.null; r.lease_expires_at=cjson.null; r.updated_at=tonumber(ARGV[3]);" });
  }
  fail({ taskId, agentId, leaseToken, error, now, backoffMs }) {
    return this.mutateOwned({ taskId, agentId, leaseToken, now,
      args: [String(error), now + backoffMs],
      mutate: "if tonumber(r.attempts)>=tonumber(r.max_attempts) then r.state='dead_letter'; r.reason='attempt_limit_exceeded'; r.next_retry_at=cjson.null else r.state='retry_wait'; r.reason='worker_failed'; r.next_retry_at=tonumber(ARGV[5]) end r.error=ARGV[4]; r.owner=cjson.null; r.lease_token=cjson.null; r.lease_expires_at=cjson.null; r.updated_at=tonumber(ARGV[3]);" });
  }
  handoff({ taskId, fromAgentId, fromLeaseToken, toAgent, now, leaseMs }) {
    ...
    return this.mutateOwned({ taskId, agentId: fromAgentId, leaseToken: fromLeaseToken, now,
      ...
      mutate: "if tonumber(r.lease_expires_at or 0)<=tonumber(ARGV[3]) then return nil end r.owner=ARGV[4]; r.agent_mode=ARGV[5]; r.cost_tier=tonumber(ARGV[6]); r.lease_token=ARGV[7]; r.lease_expires_at=tonumber(ARGV[8]); r.state='claimed'; r.updated_at=tonumber(ARGV[3]);" });
  }
  async recoverExpired(now) {
    ...
    for (const key of keys) {
      const script = `local raw=redis.call('GET',KEYS[1]); if not raw then return nil end local r=cjson.decode(raw); if (r.state=='claimed' or r.state=='running') and tonumber(r.lease_expires_at or 0)<=tonumber(ARGV[1]) then r.state='retry_wait'; r.reason='lease_expired'; r.owner=cjson.null; r.lease_token=cjson.null; r.lease_expires_at=cjson.null; r.next_retry_at=tonumber(ARGV[1]); r.updated_at=tonumber(ARGV[1]); local out=cjson.encode(r); redis.call('SET',KEYS[1],out); return out end return nil`;
      const raw = await this.command(['EVAL', script, '1', key, String(now)]);
