...
async function callClaude(model, history, identityText, memoriesText, accumulatedUsage) {
  ...
  if (!reply) {
    ...
    if (model === MODEL_TIERS.cheap) {
      return callClaude(MODEL_TIERS.standard, history, identityText, memoriesText, usage);
    }
    if (model === MODEL_TIERS.standard) {
      return callClaude(MODEL_TIERS.heavy, history, identityText, memoriesText, usage);
    }
...
export async function askNex(message, history = [], forcedTier = null) {
  ...
  const result = await callClaude(model, updatedHistory, identityText, memoriesText);
